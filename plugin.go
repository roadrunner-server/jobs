...
func (p *Plugin) Push(ctx context.Context, j jobsApi.Message) error {
	...
	if err != nil {
		...
		p.log.Error("job push error", zap.String("ID", j.ID()), zap.String("pipeline", ppl.Name()), zap.String("driver", ppl.Driver()), zap.Time("start", start), zap.Int("elapsed", time.Since(start).Milliseconds()), zap.Error(err))
		return errors.E(op, err)
	}
	...
	p.log.Debug("job was pushed successfully", zap.String("ID", j.ID()), zap.String("pipeline", ppl.Name()), zap.String("driver", ppl.Driver()), zap.Time("start", start), zap.Int("elapsed", time.Since(start).Milliseconds()))
...
func (p *Plugin) PushBatch(ctx context.Context, j []jobsApi.Message) error {
	...
	for i := 0; i < len(j); i++ {
		...
		if err != nil {
			cancel()
			p.metrics.CountPushErr()
			p.log.Error("job push batch error", zap.String("ID", j[i].ID()), zap.String("pipeline", ppl.Name()), zap.String("driver", ppl.Driver()), zap.Time("start", start), zap.Int("elapsed", time.Since(start).Milliseconds()), zap.Error(err))
			return errors.E(op, err)
		}
