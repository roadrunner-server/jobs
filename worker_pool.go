	...
	for i := 0; i < p.maxWorkers; i++ {
		go func() {
			for job := range p.queueCh {
				...
				p.log.Debug("driver ready", zap.String("pipeline", job.pipe.Name()), zap.String("driver", job.pipe.Driver()), zap.Time("start", t), zap.Int("elapsed", time.Since(start).Milliseconds()))
